...
@implementation RPDFDocumentOutline
...
{
	for (RPDFOutlineEntry *item in array) // Enumerate array entries
	{
...
+ (void)outlineItems:(CGPDFDictionaryRef)outlineDictionary document:(CGPDFDocumentRef)document array:(NSMutableArray *)array level:(NSInteger)level
{
	...
	{
		RPDFOutlineEntry *outlineEntry = nil; // An entry
		...
		if (CGPDFDictionaryGetString(outlineDictionary, "Title", &string) == true)
		{
			...
			if ((title = CGPDFStringCopyTextString(string)) != NULL) // Copy of CFObject string
			{
				...
				outlineEntry = [RPDFOutlineEntry newWithTitle:trimmed target:entryTarget level:level]; // New entry
...
+ (NSArray *)outlineFromFileURL:(NSURL *)fileURL password:(NSString *)phrase
{
	...
	if ((fileURL != nil) && [fileURL isFileURL]) // Check for valid file URL
	{
		...
		{
			outlineArray = [self outlineFromDocument:document password:phrase];
...
@implementation RPDFOutlineEntry
{
	NSInteger _level;
...
{
	return [[RPDFOutlineEntry alloc] initWithTitle:title target:target level:level];
}
